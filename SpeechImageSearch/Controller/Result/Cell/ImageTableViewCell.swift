...
class ImageTableViewCell: UITableViewCell, CellConfig {
    ...
    private var imageViewRatioLayoutConstraint: NSLayoutConstraint? {
        didSet {
            if let oldLayoutConstraint = oldValue {
                baseImageView.removeConstraint(oldLayoutConstraint)
            }
            
            if let layoutConstraint = imageViewRatioLayoutConstraint {
                baseImageView.addConstraint(layoutConstraint)
            }
        }
    }
    ...
    override func prepareForReuse() {
        ...
        imageViewRatioLayoutConstraint = nil
    }
    ...
    func setup(rowViewModel: RowViewModel) {
        ...
        let constraint = NSLayoutConstraint(item: baseImageView,
                                            attribute: .width,
                                            relatedBy: .equal,
                                            toItem: baseImageView,
                                            attribute: .height,
                                            multiplier: aspect,
                                            constant: 0)
        constraint.priority = UILayoutPriority(999)
        imageViewRatioLayoutConstraint = constraint
        
        let placeholderImageView = CenterPositionImageView(image: #imageLiteral(resourceName: "placeholder"))
        baseImageView.kf.setImage(with: URL(string: document.image_url),
