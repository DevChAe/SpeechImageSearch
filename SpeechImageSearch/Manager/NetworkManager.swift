...
class NetworkManager {
    ...
    func cancelKakaoImageSearch() {
        lastImageSearchDataRequest?.cancel()
        lastImageSearchDataRequest = nil
        lastImageSearchRequestInfo = nil
    }
    ...
    func requestKakaoImageSearchAPI(_ info: ImageSearchRequestInfo, success: @escaping (ImageSearchInfo) -> (), fail: @escaping (Error) -> ()) -> Bool {
        guard !kakaoRestApiKey.isEmpty else {
            fail(NetworkError.NonKakaoApiKey)
            return false
        }
        
        // 마지막 요청 정보 존재 확인
        if let lastInfo = lastImageSearchRequestInfo {
            // 현재 동작중인 세션이 확인
            if let dataRequest = lastImageSearchDataRequest {
        ...
        let dataRequest =
            self.manager.request(kakaoImageSearchUrl,
                                 method: .get,
        ...
        lastImageSearchRequestInfo = info
        lastImageSearchDataRequest = dataRequest
